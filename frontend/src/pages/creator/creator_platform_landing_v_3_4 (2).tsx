...
function CreatorChecklist({ onNav }: { onNav: (p: string) => void }) {
  ...
  return (
    <div className="rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 shadow-sm transition-colors">
      ...
      <button className="mt-6 w-full px-4 py-3 rounded-2xl bg-[#f77f00] text-white font-semibold hover:bg-[#e26f00] transition-all active:scale-[0.98] active:opacity-90" onClick={() => onNav("/onboarding")}>Start onboarding</button>
      <div className="mt-3 text-[12px] text-slate-500 dark:text-slate-400">Already a creator? <button className="text-[#f77f00] hover:underline" onClick={() => onNav("/home")}>Sign in</button></div>
    </div>
  );
}
...
export default function CreatorPlatformLanding({ onEnter: _onEnter }: { onEnter: () => void }) {
  ...
  const handleNav = (path: string) => {
    // Special handling for Sign In / Dashboard access
    if (path === "/home" || path === "/dashboard") {
      if (IS_EVZONE_ACCOUNTS_CONNECTED) {
        // FINAL: Redirect to external EVzone Accounts
        window.location.href = `https://accounts.evzone.app?target=${encodeURIComponent(path)}`;
        return;
      } else {
        // INTERIM: Direct access (Simulate login)
        // This avoids the "Continue to EVzone" intermediate page for now.
    ...
    const target = getLandingPageTarget(path);
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      <div className="fixed top-0 left-0 right-0 w-full z-50 bg-white/85 dark:bg-slate-950/85 backdrop-blur border-b border-slate-200 dark:border-slate-800 transition-colors">
        <div className="w-full px-4 md:px-6 py-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="hidden md:inline-flex items-center gap-2 px-4 py-2 rounded-2xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 font-semibold transition-all active:scale-[0.98] active:opacity-80" onClick={() => handleNav("/home")}>
              Sign in <ArrowRight className="h-4 w-4" />
            </button>
            <button className="hidden md:inline-flex items-center gap-2 px-4 py-2 rounded-2xl text-white font-semibold transition-all hover:opacity-90 active:scale-[0.98] active:opacity-80" style={{ background: ORANGE }} onClick={() => handleNav("/onboarding")}>
        ...
        {mobileMenu ? (
          <div className="md:hidden border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 transition-colors">
            <div className="w-full px-4 py-3 grid grid-cols-2 gap-2">
              {NAV.map((n) => (
              ...
              <button className="col-span-1 px-4 py-3 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800 font-semibold text-slate-900 dark:text-slate-100 transition-all active:scale-[0.98] active:opacity-80" onClick={() => handleNav("/home")}>Sign in</button>
              <button className="col-span-1 px-4 py-3 rounded-2xl text-white font-semibold transition-all active:scale-[0.98] active:opacity-80" style={{ background: ORANGE }} onClick={() => handleNav("/onboarding")}>Join as a Creator</button>
            </div>
          </div>
        ) : null}
      </div>
      ...
      <div id="top" className="relative overflow-hidden">
        ...
        <div className="relative w-full px-4 md:px-6 pt-24 md:pt-32 pb-8 md:pb-12">
          <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.95fr)] gap-8 items-center">
            <motion.div initial="hidden" animate="show" variants={fadeUp} transition={{ duration: 0.5 }}>
              ...
              <div className="mt-5 flex flex-col sm:flex-row gap-3">
                <button className="px-5 py-3 rounded-2xl text-white font-semibold inline-flex items-center justify-center gap-2 hover:bg-[#e26f00] transition-all active:scale-[0.98] active:opacity-90" style={{ background: ORANGE }} onClick={() => handleNav("/onboarding")}>
      ...
      {/* Features */}
      <div id="features" className="w-full px-4 md:px-6 py-12">
      ...
      {/* Studio */}
      <div id="studio" className="w-full px-4 md:px-6 py-16">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          <div>
      ...
      {/* Trust */}
      <div id="trust" className="w-full px-4 md:px-6 py-12 transition-colors">
      ...
      {/* FAQ */}
      <div id="faq" className="w-full px-4 md:px-6 py-12">
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          <div>
