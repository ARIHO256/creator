    ...
    <button
      ...
      className={cx(
        ...
        tone === 'dark'
          ...
          : 'border-slate-200/70 bg-white dark:bg-slate-900/80 text-slate-800 hover:bg-gray-50 dark:hover:bg-slate-800'
      )}
    >
      {children}
    </button>
    ...
    <div className="rounded-3xl border border-slate-200/70 bg-white dark:bg-slate-900/70 p-6">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <div className="mt-2 text-sm font-semibold text-slate-500">{message}</div>
          {action ? (
            <button
              type="button"
...
function SparkleDot() {
  return (
    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" aria-hidden="true">
      ...
      <path
        d="M10 1l1.2 5.1L16 7.3l-4.8 1.2L10 13l-1.2-4.5L4 7.3l4.8-1.2L10 1z"
        fill="currentColor"
        opacity="0.18"
      />
    </svg>
              ...
              <div className="flex items-start justify-between gap-3 border-b border-slate-200/70 p-4">
                <div>
                  ...
                  <div className="mt-1 text-xs font-semibold text-slate-500">
                    Premium UI with safe actions and clear results.
                  </div>
                </div>
      ...
      {open ? (
        <>
          ...
          <motion.aside
            ...
            exit={{ x: 30, opacity: 0 }}
            transition={{ duration: 0.18 }}
            className="fixed left-4 top-24 z-[75] w-[92vw] max-w-[360px] max-h-[calc(100vh-120px)] rounded-3xl border border-slate-200/70 bg-white dark:bg-slate-900/90 shadow-2xl backdrop-blur lg:left-[calc(var(--shell-sidebar-width)+16px)]"
          >
...
function OrdersTable({ rows, selected, setSelected, toggleAll, allVisibleSelected, onOpen }) {
  return (
    <div className="overflow-hidden rounded-[10px] border border-slate-200/70 bg-white dark:bg-slate-900">
          ...
          return (
            <div key={o.id} className="grid grid-cols-12 gap-2 px-4 py-3">
              ...
              <button
                type="button"
                onClick={() => onOpen(o.id)}
                className="col-span-3 flex items-center gap-3 rounded-2xl text-left"
              >
              ...
              <div className="col-span-1 flex items-center text-sm font-extrabold text-slate-800">
                {o.items}
              </div>
...
function OrdersList({ orders, openOrder, pushToast, offlineNotice }) {
  ...
  const kanban = useMemo(() => {
    ...
    const map = new Map(cols.map((c) => [c, []]));
  ...
  const bulkAction = (kind) => {
    if (!selectedIds.length) {
      pushToast({
        title: 'Select orders',
        message: 'Choose one or more orders first.',
        tone: 'warning',
      });
      return;
    }
    ...
    <div>
              ...
              <button
                ...
                className={cx(
                  ...
                  view === 'list'
                    ...
                    : 'text-slate-700 hover:bg-gray-50 dark:hover:bg-slate-800'
                )}
              >
                List
              </button>
          ...
          <div className="inline-flex overflow-hidden rounded-2xl border border-slate-200/70 bg-white dark:bg-slate-900/70">
            {['All', 'Watch', 'At Risk'].map((r) => (
              <button
                ...
                className={cx(
                  'px-4 py-2 text-xs font-extrabold',
                  risk === r ? 'bg-orange-50 text-orange-800' : 'text-slate-700 hover:bg-gray-50 dark:hover:bg-slate-800'
                )}
              >
                {r}
              </button>
                  ...
                  {['Confirm', 'Pack', 'Ship', 'Print', 'Batch'].map((a) => (
                    <button
                      ...
                      className={cx(
                        ...
                        a === 'Batch'
                          ? 'border border-emerald-200 bg-white dark:bg-slate-900 text-emerald-800'
                          : 'text-white'
                      )}
      ...
      <div className="mt-4">
        {view === 'kanban' ? (
            ...
            {kanban.map((col) => (
              <GlassCard key={col.status} className="p-3">
                ...
                <div className="mt-3 flex flex-col gap-2">
                  {col.items.length === 0 ? (
                    <div className="rounded-2xl border border-slate-200/70 bg-white dark:bg-slate-900/60 p-3 text-[11px] font-semibold text-slate-500">
                      No orders
                    </div>
                  ) : (
                    col.items.map((o) => (
                        ...
                        <div className="flex items-start gap-2">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2">
                              ...
                              {o.risk !== 'ok' ? (
                                <Badge tone={o.risk === 'risk' ? 'danger' : 'orange'}>
                                  {o.label}
                                </Badge>
                              ) : null}
                            </div>
                            <div className="mt-1 flex items-center gap-2">
                              ...
                              <div className="min-w-0 flex-1 truncate text-[11px] font-semibold text-slate-500">
                                {o.customer}
                              </div>
                            </div>
                            <div className="mt-2 flex items-center gap-2">
                              <Badge tone="slate">{o.warehouse}</Badge>
                              <span className="ml-auto text-[11px] font-extrabold text-slate-700">
                                {fmtMoney(o.total, o.currency)}
                              </span>
                            </div>
                          </div>
          ...
          <div className="space-y-4">
            ...
            {filtered.length === 0 ? (
              <EmptyState title="No results" message="Try adjusting filters or clearing search." />
            ) : null}
          </div>
        ) : (
          <OrdersTable
      ...
      <Modal open={exportOpen} title="Export orders" onClose={() => setExportOpen(false)}>
        <div className="grid gap-2">
          {[
            { k: 'csv', t: 'CSV', d: 'For spreadsheets and pipelines' },
            { k: 'xlsx', t: 'Excel', d: 'Formatted workbook' },
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2 text-[11px] font-extrabold text-slate-500">
                    Includes filters, SLA flags, warehouse grouping
                  </div>
                </div>
                <ChevronRight className="h-4 w-4 text-slate-300" />
              </div>
      ...
      <Drawer open={batchOpen} title="Batch by warehouse" onClose={() => setBatchOpen(false)}>
        <div className="rounded-3xl border border-slate-200/70 bg-white dark:bg-slate-900/70 p-4">
          ...
          <div className="mt-2 text-xs font-semibold text-slate-500">
            Split selected orders into fulfillment batches and print labels.
          </div>
        </div>
        ...
        <div className="mt-4 space-y-3">
          {Object.entries(
            ...
            <div key={wh} className="rounded-3xl border border-slate-200/70 bg-white dark:bg-slate-900/70 p-4">
              <div className="flex items-center gap-2">
                ...
                <span className="ml-auto">
                  <Badge tone="green">{count} orders</Badge>
                </span>
              </div>
              <div className="mt-3 flex flex-wrap gap-2">
                <button
                  ...
                  onClick={() =>
                    pushToast({
                      title: 'Wave created',
                      message: `${wh} wave planned (wire to WMS).`,
                      tone: 'success',
                    })
                  }
                ...
                <button
                  ...
                  onClick={() =>
                    pushToast({
                      title: 'Labels queued',
                      message: `${wh} labels queued (wire to printer).`,
                      tone: 'default',
                    })
                  }
          ...
          {selectedIds.length === 0 ? (
            <EmptyState
              ...
              message="Select orders in the table then open batching."
            />
          ) : null}
        </div>
    ...
    <div className="flex items-start gap-3">
      <div
        className={cx(
          ...
          done ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200/70 bg-white dark:bg-slate-900'
        )}
      >
  ...
  const prompts = useMemo(() => {
      ...
      list.push({
        ...
        action: {
          ...
          onClick: () => {
            setTab('Messages');
            setDraft(
              'Update: Your order is being prepared. Estimated ship time: today. We will share tracking shortly.'
            );
            pushToast({
              title: 'Draft created',
              message: 'ETA message draft is ready.',
              tone: 'success',
            });
          },
        },
    ...
    if (missingTracking) {
      list.push({
        ...
        message:
          "If shipped, add tracking and attach a shipping label to avoid 'Item not received' disputes.",
    ...
    if (list.length === 0) {
      list.push({
        ...
        message:
          'Everything looks healthy. Keep communication active and upload proofs as you ship.',
        action: {
          label: 'Open templates',
          onClick: () =>
            pushToast({
              title: 'Templates',
              message: 'Use message templates for faster replies.',
              tone: 'default',
            }),
        },
      });
    ...
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto flex flex-wrap items-center gap-2">
          ...
          <Badge
            tone={meta.risk === 'risk' ? 'danger' : meta.risk === 'watch' ? 'orange' : 'green'}
          >
            SLA {meta.label}
          </Badge>
        </div>
            ...
            <div className="border-b border-slate-200/70 bg-white dark:bg-slate-900/70 px-5 py-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="mt-1 text-xs font-semibold text-slate-500">
                    Order-level operations with safe actions.
                  </div>
                </div>
                    ...
                    <div className="grid gap-3">
                      <div className="grid gap-3 md:grid-cols-3">
                        ...
                        <MiniCard title="Shipping" icon={Truck}>
                          ...
                          <div className="mt-2">
                            <Badge tone="orange">Tracking pending</Badge>
                          </div>
                        </MiniCard>
                        <MiniCard title="Payment" icon={Receipt}>
                          ...
                          <div className="mt-2">
                            <Badge tone="slate">PAY-88421</Badge>
                          </div>
                        </MiniCard>
                      </div>
                      ...
                      <div className="rounded-3xl border border-slate-200/70 bg-white dark:bg-slate-900/70 p-4">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <div className="mt-1 text-xs font-semibold text-slate-500">
                              Operational steps with audit entry points.
                            </div>
                          </div>
                          <Badge
                            tone={
                          ...
                          >
                            Due {meta.mins <= 0 ? 'now' : `in ${meta.mins}m`}
                          </Badge>
                        </div>
                        <div className="mt-4 grid gap-3">
                          ...
                          <TimelineRow
                            title="Confirmed"
                            subtitle="Stock verified"
                            done={order.status !== 'New'}
                          />
                          <TimelineRow
                            title="Packed"
                            subtitle="Awaiting packing proof"
                            done={
                              order.status === 'Packed' ||
                      ...
                      <div className="rounded-3xl border border-slate-200/70 bg-white dark:bg-slate-900/70 p-4">
                        ...
                        <div className="mt-3 space-y-2 text-xs font-semibold text-slate-700">
                          <Row
                            label="Subtotal"
                            value={fmtMoney(pricing.subtotal, order.currency)}
                          />
                          <Row label="Taxes" value={fmtMoney(pricing.taxes, order.currency)} />
                          <Row
                            label="Shipping"
                            value={fmtMoney(pricing.shipping, order.currency)}
                          />
                          <div className="h-px bg-slate-200/70" />
                          <Row
                            label="Total"
                            value={fmtMoney(pricing.total, order.currency)}
                            strong
                          />
                        </div>
                      </div>
                    ...
                    <div className="rounded-3xl border border-slate-200/70 bg-white dark:bg-slate-900/70 p-4">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="mt-1 text-xs font-semibold text-slate-500">
                            Line items with unit and totals
                          </div>
                        </div>
                        ...
                        <div className="divide-y divide-slate-200/70">
                          {items.map((it) => (
                            ...
                            >
                              ...
                              <div className="col-span-2 flex items-center">
                                {fmtMoney(it.unit, order.currency)}
                              </div>
                              <div className="col-span-2 flex items-center font-black text-slate-900">
                                {fmtMoney(it.qty * it.unit, order.currency)}
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    <div className="grid gap-3">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="mt-1 text-xs font-semibold text-slate-500">
                            Templates, auto-translation, and fast replies.
                          </div>
                        </div>
                        <button
                          type="button"
                        ...
                        {messages.map((m) => (
                          <div
                            ...
                            className={cx(
                              ...
                              m.from === 'supplier'
                                ? 'ml-auto border-emerald-200 bg-emerald-50/60'
                                : 'mr-auto border-slate-200/70 bg-white dark:bg-slate-900'
                            )}
                          >
                            <div className="flex items-center gap-2">
                              <Badge tone={m.from === 'supplier' ? 'green' : 'slate'}>
                                {m.from === 'supplier' ? 'You' : 'Buyer'}
                              </Badge>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {[
                          'Thanks, we are preparing it now.',
                          'We will share tracking within 2 hours.',
                          'Your order is packed. Shipping shortly.',
                        ].map((tpl) => (
                          <button
                            key={tpl}
                            type="button"
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            pushToast({
                              title: 'Message sent',
                              message: 'Buyer updated (wire to messaging).',
                              tone: 'success',
                            });
                            setDraft('');
                          }}
                    ...
                    <div className="grid gap-3">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <div className="mt-1 text-xs font-semibold text-slate-500">
                            Invoices, packing photos, and shipping labels.
                          </div>
                        </div>
                        <button
                          type="button"
                      ...
                      <div className="space-y-2">
                        {proofs.map((p) => (
                          <div
                            key={p.id}
                            className="flex items-center gap-3 rounded-3xl border border-slate-200/70 bg-white dark:bg-slate-900 p-3"
                          >
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="mt-0.5 text-[11px] font-semibold text-slate-500">
                                Uploaded {shortTime(p.uploadedAt)}
                              </div>
                            </div>
                            <button
                              type="button"
                        ...
                        {proofs.length === 0 ? (
                          <EmptyState
                            title="No proofs yet"
                            message="Upload invoices, packing photos, and labels."
                          />
                        ) : null}
                      </div>
                    </div>
            ...
            <div className="divide-y divide-slate-200/70">
              ...
              {filtered.length === 0 ? (
                <div className="p-5">
                  <EmptyState
                    title="No RMAs"
                    message="Create an RMA or adjust filters."
                    action={{ label: 'Create RMA', onClick: () => setDrawer(true) }}
                  />
                </div>
              ) : null}
            </div>
          ...
          <div className="mt-4 space-y-3">
            <div className="rounded-3xl border border-slate-200/70 bg-white dark:bg-slate-900/70 p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="mt-1 text-xs font-semibold text-slate-500">
                    Auto-check windows, condition, and category rules
                  </div>
                </div>
                <button
                  type="button"
            ...
            <div className="rounded-3xl border border-slate-200/70 bg-white dark:bg-slate-900/70 p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="mt-1 text-xs font-semibold text-slate-500">
                    Auto-restock on received and passed inspection
                  </div>
                </div>
                <button
                  type="button"
            ...
            <div className="rounded-3xl border border-slate-200/70 bg-white dark:bg-slate-900/70 p-4">
              <div className="flex items-center gap-2">
                ...
                <span className="ml-auto">
                  <Badge tone="slate">Last 30 days</Badge>
                </span>
              </div>
...
function Disputes({ disputesList, pushToast }) {
  ...
  return (
    <div>
      ...
      <Modal
        open={playbooksOpen}
        title={`Playbooks${active?.type ? ` · ${active.type}` : ''}`}
        ...
        <div className="grid gap-3">
          ...
          {visiblePlaybooks.length === 0 ? (
            <EmptyState
              ...
              message="Try a different dispute or clear filters."
            />
          ) : null}
        </div>
      </Modal>
      ...
      <div className="grid gap-4 lg:grid-cols-12">
        <GlassCard className="p-5 lg:col-span-7">
          ...
          <div className="mt-4 space-y-2">
            ...
            {filtered.length === 0 ? (
              <EmptyState title="No disputes" message="Try changing filters or clearing search." />
            ) : null}
          </div>
        </GlassCard>
        ...
        <GlassCard className="p-5 lg:col-span-5">
          {active ? (
            <>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="mt-1 text-xs font-semibold text-slate-500">
                    Evidence, actions, and export pack
                  </div>
                </div>
            ...
            <EmptyState
              title="Select a case"
              message="Choose a dispute from the list to view details."
            />
          )}
        </GlassCard>
      </div>
