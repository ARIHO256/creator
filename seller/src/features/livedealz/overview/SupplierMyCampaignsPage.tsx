...
const STAGE_FROM_BACKEND_STATUS = {
  DRAFT: "Draft",
  OPEN: "Collabs",
...
function toYmdFromDateLike(value, fallback = "") {
...
function normalizeCampaignItem(item, currency) {
  const entry = item && typeof item === "object" && !Array.isArray(item) ? item : {};
  const price = Math.max(0, Number(entry.price || 0));
  const discountMode = entry.discount?.mode || "none";
  const discountValue = Number(entry.discount?.value || 0);
  const discountedPrice = calcDiscountedPrice(price, discountMode, discountValue);
  return {
    id: String(entry.id || uid("I")),
    title: String(entry.title || "Campaign item"),
    subtitle: String(entry.subtitle || ""),
    kind: String(entry.kind || "Product"),
    category: String(entry.category || ""),
    region: String(entry.region || ""),
    sku: String(entry.sku || ""),
    price,
    plannedQty: Math.max(1, Number(entry.plannedQty || 1)),
    discount: {
      mode: discountMode,
      value: discountValue
    },
    discountedPrice,
    discountLabel: String(entry.discountLabel || formatDiscount(discountMode, discountValue, currency)),
    avatar: entry.avatar || svgAvatarDataUrl(entry.title || "I", entry.id || entry.sku || "I"),
    assetId: String(entry.assetId || ""),
    assetTitle: String(entry.assetTitle || ""),
    assetDimensions: String(entry.assetDimensions || ""),
    assetPreview: entry.assetPreview || ""
  };
}

function normalizeWorkspaceCampaignRecord(raw) {
  const record = raw && typeof raw === "object" && !Array.isArray(raw) ? raw : {};
  const metadata = record.metadata && typeof record.metadata === "object" && !Array.isArray(record.metadata) ? record.metadata : {};
  const now = Date.now();
  const name = String(record.title || metadata.name || "Untitled campaign").trim();
  const currency = String(record.currency || metadata.currency || "USD");
  const startDate = String(metadata.startDate || "").trim() || toYmdFromDateLike(record.startAt, todayYMD());
  const parsedDuration = Number(metadata.durationDays || 0);
  const durationDays = Number.isFinite(parsedDuration) && parsedDuration >= 1 ? Math.floor(parsedDuration) : 7;
  const endDate = String(metadata.endDate || "").trim() || computeEndDate(startDate, durationDays);
  const status = String(record.status || "").toUpperCase();
  const normalizedStage = STAGES.includes(metadata.stage)
    ? metadata.stage
    : STAGE_FROM_BACKEND_STATUS[status] || "Draft";
  const approvalStatus = String(
    metadata.approvalStatus ||
      (status === "CANCELLED" ? "Rejected" : status === "COMPLETED" ? "Approved" : "NotSubmitted")
  );

  return hydrateCampaignGiveaway({
    id: String(record.id || uid("S")),
    name,
    description: String(record.description || metadata.description || ""),
    title: name,
    type: String(metadata.type || "Shoppable Adz"),
    locationType: String(metadata.locationType || "Region"),
    locationValue: String(metadata.locationValue || metadata.region || "East Africa"),
    region: String(metadata.region || metadata.locationValue || "East Africa"),
    currency,
    estValue: Number(record.budget ?? metadata.estValue ?? 0) || 0,
    startDate,
    durationDays,
    endDate,
    offerScope: String(metadata.offerScope || "Products"),
    promoType: String(metadata.promoType || "Discount"),
    promoArrangement: String(metadata.promoArrangement || "PercentOff"),
    promoCode: String(metadata.promoCode || ""),
    shippingThreshold: Number(metadata.shippingThreshold || 0),
    giftNote: String(metadata.giftNote || ""),
    items: Array.isArray(metadata.items) ? metadata.items.map((item) => normalizeCampaignItem(item, currency)) : [],
    giveawayExtras: Array.isArray(metadata.giveawayExtras)
      ? metadata.giveawayExtras.map((item) => ensureGiveawayExtra(normalizeCampaignItem(item, currency)))
      : [],
    ...
    allowMultiCreators: metadata.allowMultiCreators !== false,
    approvalStatus,
    pendingAdminApproval:
      Boolean(metadata.pendingAdminApproval) || approvalStatus === "Pending",
    pendingSupplierApproval: Boolean(metadata.pendingSupplierApproval),
    adminRejected: Boolean(metadata.adminRejected),
    creatorRejected: Boolean(metadata.creatorRejected),
    stage: normalizedStage,
    queuedStageAfterApproval: String(metadata.queuedStageAfterApproval || "Collabs"),
    queuedNextActionAfterApproval: String(metadata.queuedNextActionAfterApproval || "Await pitches"),
    creatorsCount: Number(metadata.creatorsCount || 0),
    pitchesCount: Number(metadata.pitchesCount || 0),
    invitesSent: Number(metadata.invitesSent || 0),
    invitesAccepted: Number(metadata.invitesAccepted || 0),
    proposalsCount: Number(metadata.proposalsCount || 0),
    contractCount: Number(metadata.contractCount || 0),
    ...
    terminationExplanation: metadata.terminationExplanation || "",
    terminatedAt: metadata.terminatedAt || null,
    terminationRequestedAt: metadata.terminationRequestedAt || null
  });
}

function buildCampaignPersistencePayload(campaign, options = {}) {
  const status = String(options.status || campaign.status || "DRAFT").toUpperCase();
  const metadata = {
    ...campaign,
    id: campaign.id
  };
  return {
    ...
    startAt: campaign.startDate ? `${campaign.startDate}T00:00:00.000Z` : undefined,
    endAt: campaign.endDate ? `${campaign.endDate}T23:59:59.000Z` : undefined,
    metadata
  };
}
...
export default function SupplierMyCampaignsPage() {
  ...
  async function loadCampaignWorkspace() {
    ...
    try {
      ...
      const rows = Array.isArray(workspace?.campaigns) ? workspace.campaigns : [];
  ...
  async function submitTerminationFlow() {
    ...
    try {
      if (hasActiveContracts) {
        const patchPayload = buildCampaignPersistencePayload(
          {
            ...targetCampaign,
            health: "at-risk",
        ...
        const updated = await sellerBackendApi.patchCampaign(targetCampaign.id, patchPayload);
        const normalized = normalizeWorkspaceCampaignRecord(updated);
        setCampaigns((xs) => xs.map((x) => (x.id === normalized.id ? normalized : x)));
      ...
      const message = error instanceof Error && error.message ? error.message : "Failed to update campaign termination.";
  ...
  async function upsertCampaign({ submitForApproval }) {
    ...
    try {
      const payload = buildCampaignPersistencePayload(newCampaign, {
        status: submitForApproval ? "OPEN" : "DRAFT"
      });
      const created = await sellerBackendApi.createCampaign(payload);
      const normalized = normalizeWorkspaceCampaignRecord(created);
      setCampaigns((xs) => [normalized, ...xs.filter((x) => x.id !== normalized.id)]);
      ...
      const message = error instanceof Error && error.message ? error.message : "Failed to save campaign.";
      push(message, "error");
    }
  }

  async function simulateAdminDecision(c, decision) {
    if (decision === "approve") {
      try {
        const updated = await sellerBackendApi.patchCampaign(
          c.id,
          buildCampaignPersistencePayload(
            {
              ...c,
              approvalStatus: "Approved",
              pendingAdminApproval: false,
        ...
        const message = error instanceof Error && error.message ? error.message : "Failed to approve campaign.";
        push(message, "error");
      }
      return;
    }
    ...
    try {
      ...
      const message = error instanceof Error && error.message ? error.message : "Failed to reject campaign.";
      push(message, "error");
    }
  }

  async function resubmitAfterRejection(c) {
    try {
      const updated = await sellerBackendApi.patchCampaign(
        c.id,
        buildCampaignPersistencePayload(
      ...
      const normalized = normalizeWorkspaceCampaignRecord(updated);
      setCampaigns((xs) => xs.map((x) => (x.id === normalized.id ? normalized : x)));
      push("Resubmitted for approval.", "success");
    } catch (error) {
      const message = error instanceof Error && error.message ? error.message : "Failed to resubmit campaign.";
      push(message, "error");
    }
  }
