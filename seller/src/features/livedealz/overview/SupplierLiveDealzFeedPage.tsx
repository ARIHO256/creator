...
function normalizeLiveFeedWorkspace(raw) {
  const workspace = asRecord(raw);
  const hero = asRecord(workspace.hero);
  const crew = asRecord(workspace.crew);
  const pipeline = asRecord(workspace.pipeline);

  return {
    supplierActsAsCreator: asBoolean(workspace.supplierActsAsCreator),
    hero: {
      initials: asString(hero.initials),
      name: asString(hero.name),
      subtitle: asString(hero.subtitle),
      tier: asString(hero.tier),
      kpis: asArray(hero.kpis).map((entry) => {
    ...
    todayItems: asArray(workspace.todayItems).map((entry) => {
      const item = asRecord(entry);
      return {
        time: asString(item.time),
        label: asString(item.label),
        badge: asString(item.badge),
        badgeColor: asString(item.badgeColor)
      };
    }),
    feedItems: asArray(workspace.feedItems).map((entry) => {
      const item = asRecord(entry);
      const rawType = asString(item.type).toLowerCase();
      const type =
        rawType === "live" || rawType === "upcoming" || rawType === "replay"
          ? rawType
          : "upcoming";
      const fallbackId = `${type}-${asString(item.title)}-${asString(item.time)}`;
      return {
        id: asString(item.id) || fallbackId,
        type,
        title: asString(item.title),
        brand: asString(item.brand),
        viewers: asString(item.viewers),
        time: asString(item.time),
        tag: asString(item.tag)
      };
    }),
    followedCreators: asArray(workspace.followedCreators).map((entry) => {
      const item = asRecord(entry);
      return {
        id: asString(item.id),
        ...
        viewers:
          item.viewers === null || item.viewers === undefined
            ? null
            : asNumber(item.viewers, 0)
      };
    }),
    pipeline: {
      stages: asArray(pipeline.stages).map((entry) => {
        const item = asRecord(entry);
        return {
          label: asString(item.label),
          value: String(item.value ?? ""),
          amount: asString(item.amount),
          progressPct: Math.max(0, Math.min(100, asNumber(item.progressPct, 0))),
          highlight: asBoolean(item.highlight)
        };
      })
    },
    crew: {
      title: asString(crew.title),
      rows: asArray(crew.rows).map((entry) => {
        const item = asRecord(entry);
        return {
    ...
    aiSuggestions: asArray(workspace.aiSuggestions).map((entry) => {
      const item = asRecord(entry);
      return {
        title: asString(item.title),
    ...
    categoryInsights: asArray(workspace.categoryInsights).map((entry) => {
      const item = asRecord(entry);
      const tone = asString(item.badgeTone);
      return {
        label: asString(item.label),
        badge: asString(item.badge),
        badgeColor:
  ...
  return (
    <div className="flex-1 min-w-0 flex flex-col gap-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-2 text-sm">
        {kpis.map((kpi) => (
          <HeroKpi
            key={`${asString(kpi.label)}-${asString(kpi.value)}`}
            label={asString(kpi.label)}
            value={String(kpi.value ?? "")}
            sub={asString(kpi.sub)}
          />
        ))}
      </div>
    ...
    <div className="w-full xl:w-[32rem] xl:shrink-0 xl:pl-4 xl:border-l border-slate-200 dark:border-slate-800 flex flex-col justify-between text-sm">
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-xs text-slate-500 dark:text-slate-300">{dayLabel}</span>
        </div>
        <ul className="space-y-2">
          {items.map((item) => (
            <TimelineItem
              key={`${asString(item.time)}-${asString(item.label)}`}
              time={asString(item.time)}
              label={asString(item.label)}
              badge={asString(item.badge)}
              badgeColor={asString(item.badgeColor)}
              ...
              <div className="flex items-center justify-between gap-2 mb-1 w-full">
                <div className="flex flex-col">
                  <span className="font-semibold dark:font-bold text-slate-800 dark:text-slate-50 truncate">
                    {asString(c.name).startsWith("@") ? asString(c.name) : `@${asString(c.name)}`}
                  </span>
                  <span className="text-xs text-slate-500 dark:text-slate-300">
                    {c.type} · {c.category}
                  </span>
  ...
  return (
    <div className="border border-slate-100 dark:border-slate-800 rounded-2xl p-3 bg-gray-50 dark:bg-slate-950 dark:bg-slate-900/60 text-sm transition-colors">
      ...
      <div className="space-y-2">
        {stages.map((s) => (
          <PipelineStage
            key={`${asString(s.label)}-${String(s.value ?? "")}`}
            label={asString(s.label)}
            value={String(s.value ?? "")}
            amount={asString(s.amount)}
            progressPct={Math.max(0, Math.min(100, asNumber(s.progressPct, 0)))}
            highlight={asBoolean(s.highlight)}
          />
        ))}
      </div>
    </div>
  ...
  return (
    <div className="border border-slate-100 dark:border-slate-800 rounded-2xl p-3 bg-white dark:bg-slate-900 text-sm transition-colors">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-xs text-slate-500 dark:text-slate-300">{asString(crew?.title)}</span>
      </div>
      <ul className="space-y-1.5">
        {rows.map((row) => (
          <CrewRow
            key={`${asString(row.role)}-${asString(row.name)}`}
...
function AIAssistantCard({ onAsk, suggestions }) {
  return (
      ...
      <ul className="space-y-2">
        {suggestions.map((entry) => (
          <AIAssistantSuggestion
            key={`${asString(entry.title)}-${asString(entry.body)}`}
...
function CategoryInsightsCard({ onChangePage, insights }) {
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl transition-colors shadow-sm p-4 text-sm">
      <div className="flex items-center justify-between mb-2">
      ...
      <div className="space-y-2">
        {insights.map((insight) => (
          <CategoryRow
            key={`${asString(insight.label)}-${asString(insight.badge)}`}
            label={asString(insight.label)}
            badge={asString(insight.badge)}
            badgeColor={asString(insight.badgeColor)}
          />
        ))}
      </div>
      <button
...
export default function SupplierLiveDealzFeedPage() {
  ...
  const loadWorkspace = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      setWorkspace(normalizeLiveFeedWorkspace(response));
    } catch (error) {
      const message =
  ...
  const toggleFollowCreator = useCallback(
    ...
    [loadWorkspace, showSuccess, showWarning]
  );
  ...
  const filteredFeed = useMemo(
    () =>
      workspace.feedItems.filter((item) => {
          ...
          <section className="bg-white dark:bg-slate-900 rounded-2xl transition-colors shadow-sm p-4">
            <div className="flex flex-col xl:flex-row gap-4 items-start">
              <HeroSummaryCard
                ...
                hero={workspace.hero}
              />
              <TodayAtGlanceCard items={workspace.todayItems} />
            </div>
          </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-start">
            ...
            <section className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl transition-colors shadow-sm p-4 flex flex-col gap-3">
              ...
              <FollowedCreatorsStrip
                creators={workspace.followedCreators}
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                <PipelineCard pipeline={workspace.pipeline} />
                <CrewCard onChangePage={onChangePage} crew={workspace.crew} />
              </div>
            </section>
            ...
            <section className="space-y-3">
              ...
              <AIAssistantCard
                ...
                onAsk={() => showInfo("Supplier AI Assistant opened.")}
              />
              <CategoryInsightsCard onChangePage={onChangePage} insights={workspace.categoryInsights} />
            </section>
          </div>
        </div>
      </main>
