...
const DEFAULT_HERO_IMAGE = "https://images.unsplash.com/photo-1520975958225-82284e3d2e52?auto=format&fit=crop&w=1200&q=60";
...
function normalizeRole(value) {
  return asString(value) === "Supplier" ? "Supplier" : "Creator";
}

function normalizeStatus(value) {
  const status = asString(value);
  if (["Draft", "Scheduled", "Ready", "Live", "Ended"].includes(status)) return status;
  return "Draft";
}
...
function normalizeLiveDashboardWorkspace(raw) {
  ...
  const suppliers = asArray(payload.suppliers).map((entry, index) => {
    ...
    return {
      id: asString(supplier.id, `supplier_${index + 1}`),
      name: asString(supplier.name, "Supplier"),
      kind: asString(supplier.kind, "Seller"),
      avatarUrl: asString(supplier.avatarUrl)
    };
  });
  const campaigns = asArray(payload.campaigns).map((entry, index) => {
    const campaign = asRecord(entry);
    return {
      id: asString(campaign.id, `campaign_${index + 1}`),
      supplierId: asString(campaign.supplierId),
      name: asString(campaign.name, "Campaign"),
      creatorUsage: asString(campaign.creatorUsage, "I will use a Creator"),
      collabMode: asString(campaign.collabMode, "Open for Collabs"),
      approvalMode: asString(campaign.approvalMode, "Manual")
    };
  });
  const hosts = asArray(payload.hosts).map((entry, index) => {
    const host = asRecord(entry);
    const handle = asString(host.handle, "@host");
    return {
      id: asString(host.id, `host_${index + 1}`),
      name: asString(host.name, "Host"),
      handle: handle.startsWith("@") ? handle : `@${handle}`,
      role: normalizeRole(host.role),
      avatarUrl: asString(host.avatarUrl)
    };
  });
  const nowISO = new Date().toISOString();
  const sessions = asArray(payload.sessions).map((entry, index) => {
    const session = asRecord(entry);
    const startISO = normalizeIso(session.startISO, nowISO);
    const endISO = normalizeIso(session.endISO, new Date(Date.now() + 60 * 60 * 1000).toISOString());
    return {
      id: asString(session.id, `session_${index + 1}`),
      title: asString(session.title, "Live Session"),
      status: normalizeStatus(session.status),
      supplierId: asString(session.supplierId),
      campaignId: asString(session.campaignId) || null,
      hostId: asString(session.hostId),
      hostRole: normalizeRole(session.hostRole),
      platforms: asArray(session.platforms).map((item) => asString(item)).filter(Boolean),
      heroImageUrl: asString(session.heroImageUrl, DEFAULT_HERO_IMAGE),
      heroVideoUrl: asString(session.heroVideoUrl),
      desktopMode: asString(session.desktopMode, "modal"),
      startISO,
      endISO,
      peakViewers: asNumber(session.peakViewers, 0),
      avgWatchMin: asNumber(session.avgWatchMin, 0),
      chatRate: asNumber(session.chatRate, 0),
      gmv: asNumber(session.gmv, 0),
      crewConflicts: asNumber(session.crewConflicts, 0)
    };
  });
  return { sessions, suppliers, campaigns, hosts };
}
...
function NewLiveSessionDrawer({ open, onClose, suppliers, campaigns, hosts, onCreate }) {
  ...
  const availableSuppliers = useMemo(() => suppliers || [], [suppliers]);
  ...
  const scopedHosts = useMemo(() => {
    ...
    if (usage === "I will NOT use a Creator") return availableHosts.filter((h) => h.role === "Supplier");
    if (usage === "I will use a Creator") return availableHosts.filter((h) => h.role === "Creator");
    return availableHosts;
  }, [availableHosts, selectedCampaign]);

  useEffect(() => {
    if (!scopedHosts.find((h) => h.id === hostId)) setHostId(scopedHosts[0]?.id || "");
  }, [scopedHosts, hostId]);
  ...
  const handleCreateAndOpenBuilder = () => {
    if (!supplierId || !hostId || !title.trim()) return;
    ...
    const host = scopedHosts.find((h) => h.id === hostId);
    onCreate({
      id,
      title: title.trim(),
      supplierId,
  ...
  return (
    <Drawer open={open} onClose={onClose} title="New Live Session" subtitle="Create a session and jump straight into Live Builder (Supplier).">
          ...
          <div className="grid gap-3">
            <div>
              ...
              {!availableSuppliers.length ? (
                <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">No supplier workspace found.</div>
              ) : null}
            </div>
      ...
      <div className="mt-4 flex items-center justify-end gap-2">
        ...
        <PrimaryButton onClick={handleCreateAndOpenBuilder} disabled={!availableSuppliers.length || !scopedHosts.length || !title.trim()}>
          Create & Open Builder
        </PrimaryButton>
      </div>
    </Drawer>
...
export default function SupplierLiveDashboardPage() {
  ...
  async function loadWorkspace() {
    try {
      ...
      setToolSessionId((prev) => {
        if (prev && workspace.sessions.some((item) => item.id === prev)) return prev;
  ...
  const viewersTrend = useMemo(() => {
    const ordered = [...sessions].sort((a, b) => new Date(a.startISO).getTime() - new Date(b.startISO).getTime());
    const values = ordered.map((session) => Math.max(0, Number(session.peakViewers || 0)));
    if (!values.length) return [0, 0, 0, 0];
    const tail = values.slice(-14);
    if (tail.length >= 4) return tail;
    const padded = [...tail];
    while (padded.length < 4) padded.unshift(0);
    return padded;
  }, [sessions]);
