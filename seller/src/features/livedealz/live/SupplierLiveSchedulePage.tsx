...
function normalizeHostRole(value) {
  ...
  return normalized === "supplier" ? "Supplier" : "Creator";
}

function normalizeStatus(value) {
  const normalized = asString(value).toLowerCase();
  if (normalized === "confirmed" || normalized === "scheduled" || normalized === "ready") return "Confirmed";
  if (normalized === "live") return "Live";
  if (normalized === "ended") return "Ended";
  return "Draft";
}

function normalizeWeekday(value, startISO) {
  const weekday = asString(value);
  if (WEEK_DAY_SET.has(weekday)) return weekday;
  const parsed = startISO ? new Date(startISO) : null;
...
function formatDateLabel(startISO, fallback) {
  if (fallback) return fallback;
  if (!startISO) return "TBD";
  const parsed = new Date(startISO);
  if (!Number.isFinite(parsed.getTime())) return "TBD";
  return parsed.toLocaleDateString("en", { weekday: "short", month: "short", day: "numeric" });
}

function formatTimeLabel(startISO, endISO, fallback) {
  if (fallback) return fallback;
  if (!startISO) return "TBD";
  const start = new Date(startISO);
  if (!Number.isFinite(start.getTime())) return "TBD";
  const end = endISO ? new Date(endISO) : new Date(start.getTime() + 90 * 60_000);
  return `${start.toLocaleTimeString("en", { hour: "2-digit", minute: "2-digit", hour12: false })}-${end.toLocaleTimeString("en", { hour: "2-digit", minute: "2-digit", hour12: false })}`;
}

function normalizeSession(raw, index) {
  const record = asRecord(raw) || {};
  const startISO = normalizeIso(record.startISO) || normalizeIso(record.scheduledAt);
  const durationMin = Math.max(30, Math.round(asNumber(record.durationMin, 90)));
  const computedEndISO = startISO ? new Date(new Date(startISO).getTime() + durationMin * 60_000).toISOString() : "";
  const endISO = normalizeIso(record.endISO) || computedEndISO;
  const hostRole = normalizeHostRole(record.hostRole);
  const status = normalizeStatus(record.status);
  const role = hostRole === "Supplier" ? "Host (Supplier)" : "Supplier (Sponsor)";

  return {
    id: asString(record.id) || `session-${index + 1}`,
    title: asString(record.title) || "Live Session",
    campaign: asString(record.campaign) || "Campaign",
    supplier: asString(record.supplier) || "Supplier",
    host: asString(record.host) || "@host",
    hostRole,
    creatorUsage: asString(record.creatorUsage) || "I will use a Creator",
    collabMode: asString(record.collabMode) || "Open for Collabs",
    approvalMode: asString(record.approvalMode) || "Manual",
    weekday: normalizeWeekday(record.weekday, startISO),
    dateLabel: formatDateLabel(startISO, asString(record.dateLabel)),
    time: formatTimeLabel(startISO, endISO, asString(record.time)),
    location: asString(record.location) || "MyLiveDealz",
    simulcast: asString(record.simulcast),
    status,
    role,
    durationMin,
    scriptsReady: asBoolean(record.scriptsReady),
    assetsReady: asBoolean(record.assetsReady),
    productsCount: Math.max(0, Math.round(asNumber(record.productsCount, 0))),
    workloadScore: Math.max(0, Math.round(asNumber(record.workloadScore, 0))),
    conflict: asBoolean(record.conflict),
    startISO,
    endISO
  };
}

function normalizeAiSlot(raw, index) {
  const record = asRecord(raw) || {};
  return {
    id: asString(record.id) || `slot-${index + 1}`,
    label: asString(record.label) || "Suggested slot",
    reason: asString(record.reason) || "Based on your schedule performance.",
    recommendedFor: asString(record.recommendedFor) || "Upcoming lives"
  };
}

function normalizeScheduleWorkspace(raw) {
  const record = asRecord(raw) || {};
  return {
    sessions: asArray(record.sessions).map((session, index) => normalizeSession(session, index)),
    aiSlots: asArray(record.aiSlots).map((slot, index) => normalizeAiSlot(slot, index))
  };
}
...
export default function SupplierLiveScheduleCalendarPage() {
  ...
  useEffect(() => {
    ...
    const next = sessions.find((entry) => entry.id === selectedSession.id) || null;
    setSelectedSession(next);
  }, [sessions, selectedSession]);

  useEffect(() => {
    if (!rescheduleSession) return;
    const next = sessions.find((entry) => entry.id === rescheduleSession.id) || null;
          ...
          <section>
            ...
            <div className="space-y-2">
              {aiSlots.length ? aiSlots.map((slot) => (
                ...
                <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-950 dark:bg-slate-800 p-3 text-xs text-slate-500 dark:text-slate-300">
                  No AI slot suggestions are available for this workspace yet.
                </div>
              )}
            </div>
          </section>
