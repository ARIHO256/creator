...
function getHostRole(ad: Ad): HostRole {
  return ad.hostRole || "Creator";
}
function getCreatorUsage(ad: Ad): CreatorUsageDecision {
  return ad.creatorUsage || "I will use a Creator";
}
function getCollabMode(ad: Ad): CollaborationMode {
  return ad.collabMode || "Open for Collabs";
}
function getApprovalMode(ad: Ad): ApprovalMode {
  return ad.approvalMode || "Manual";
}
function hostRoleLabel(ad: Ad) {
  return getHostRole(ad) === "Supplier" ? "Supplier-hosted" : "Creator-hosted";
}
...
function SupplierGovernancePills({ ad }: { ad: Ad }) {
  const hostRole = getHostRole(ad);
  const creatorUsage = getCreatorUsage(ad);
  const collabMode = getCollabMode(ad);
  const approvalMode = getApprovalMode(ad);

  return (
    <>
      <Pill tone={hostRoleTone(hostRole)}>
        <Sparkles className="h-3.5 w-3.5" />
        {hostRoleLabel(ad)}
      </Pill>
      <Pill tone={creatorUsageTone(creatorUsage)}>
      ...
      <Pill tone={approvalTone(approvalMode)}>
...
export default function SupplierAdzMarketplace() {
    ...
    return ads
      ...
      .filter((a) => {
        ...
        return (
          ...
          getHostRole(a).toLowerCase().includes(q) ||
          getCreatorUsage(a).toLowerCase().includes(q) ||
          getCollabMode(a).toLowerCase().includes(q) ||
          getApprovalMode(a).toLowerCase().includes(q)
        );
      })
  ...
  useEffect(() => {
    if (!selected) return;
    if (!selected.offers.some((offer) => offer.id === selectedHeroOfferId)) {
        ...
        rightContent={
          <div className="flex items-center gap-2">
            <button
              ...
              onClick={() => navigate("/supplier/deliverables/assets")}
            >
              ...
              <div className="p-4 border-b border-neutral-200 dark:border-slate-800 flex flex-col gap-3 md:flex-row md:items-center md:justify-between transition-colors">
                <div className="min-w-0">
                  ...
                  <div className="mt-1 text-[11px] text-neutral-600 dark:text-slate-400">
                    This is the Shoppable Ad preview buyers will experience (phone-sized, scrollable). Supplier governance stays outside the phone preview.
                  </div>
                  <div className="mt-2 flex flex-wrap gap-2">
                    <SupplierGovernancePills ad={selected} />
                  </div>
                </div>
