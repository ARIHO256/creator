...
export default function CreatorReviewsDashboardPage() {
  ...
  const queryCreatorName = (params.get("creatorName") || "").trim();
  ...
  useEffect(() => {
    ...
    run(
      ...
      if (!result) {
        ...
        setLoadError("Could not load reviews right now. Check your connection and retry.");
        setInitialLoadDone(true);
        return;
      }

      setAllReviews(result.reviews);
      setCreatorIdentity(result.creator);
      setLoadError(null);
      setInitialLoadDone(true);
    });
  ...
  }, [queryCreatorId, queryCreatorName, reloadNonce, run]);

  const creatorScopedReviews = useMemo(() => {
    if (!queryCreatorId) return allReviews;
    return allReviews.filter((review) => review.creatorId === queryCreatorId);
  }, [allReviews, queryCreatorId]);

  const filteredReviews = useMemo(() => {
    const now = new Date();
    return creatorScopedReviews.filter((review) => {
      if (scope === "public" && !review.publicReview) return false;
      if (scope === "private" && review.publicReview) return false;
      ...
      if (timeWindow !== "all") {
        ...
        if (Number.isFinite(diff) && diff > days * 24 * 60 * 60 * 1000) return false;
      }
      ...
      if (search.trim()) {
        ...
        const haystack = [
          ...
          ...review.quickTags,
          ...review.issueTags,
          formatIntent(review.transactionIntent),
        ]
          .join(" ")
          .toLowerCase();
  ...
  }, [creatorScopedReviews, minRating, scope, search, timeWindow]);
  ...
  const availableCreators = useMemo(() => {
    ...
    allReviews.forEach(r => map.set(r.creatorId, r.creatorName));
              ...
              <SectionCard title="Filter reviews" subtitle="Find feedback by visibility, timeframe, rating, or keyword.">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {isOwner && availableCreators.length > 0 && (
                      <div className="ml-2 inline-flex items-center gap-2 rounded-full border border-orange-200 bg-orange-50 px-3 py-1.5 text-xs text-orange-700 dark:border-orange-900/40 dark:bg-orange-950/20 dark:text-orange-300">
                        <UserRound className="h-3.5 w-3.5" />
                        <span className="font-semibold">Team View:</span>
                        <select
                          value={queryCreatorId || "all"}
                          onChange={(e) => {
                            const val = e.target.value;
                            const next = new URLSearchParams(location.search);
                            if (val === "all") next.delete("creatorId");
                            else next.set("creatorId", val);
